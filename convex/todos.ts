...
import { getEmbeddingsWithAI } from "./myAi";
...
export const todayTodos = query({
  ...
  handler: async (ctx) => {
    ...
    return await ctx.db.query("todos")
      .filter((q) => q.eq(q.field("userId"), userId))
      .filter((q) => q.gte(q.field("dueDate"), todayStart.valueOf()))
      .filter((q) => q.lte(q.field("dueDate"), todayEnd.valueOf()))
      .collect();
  }
  return [];
  },
...
export const createATodo = mutation({
  ...
  return null;
      ...
      console.log("Error occured during createTodo mutation", err);
      return null;
    }
  },
});
